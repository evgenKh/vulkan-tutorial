...
VKAPI_ATTR VkBool32 VKAPI_CALL VulkanDriver::debugCallback(
...
void VulkanDriver::initVulkan(uint32_t width, uint32_t height, GLFWwindow* glfwWindow)
    {
        this->width = width;
        this->height = height;
        ...
        imageUtils.createTextureImage(textureImage, textureImageMemory);
        textureImageView = imageUtils.createImageView(textureImage, VK_FORMAT_R8G8B8A8_SRGB, VK_IMAGE_ASPECT_COLOR_BIT);
    ...
    void VulkanDriver::createInstance()
    {
        ...
        auto extensions = getRequiredExtensionsInstance();
    ...
    std::vector<const char*> VulkanDriver::getRequiredExtensionsInstance()
    {
    ...
    void VulkanDriver::createSurface(GLFWwindow* glfwWindow)
    {
    ...
    void VulkanDriver::createGraphicsPipeline()
    {
        ...
        std::vector<char> fragShaderCode = readFile("data/shaders/shader_frag.spv");
        ...
        rasterizer.polygonMode = VK_POLYGON_MODE_FILL; //VK_POLYGON_MODE_LINES
        ...
        pipelineInfo.layout = pipelineLayout;//Fixed func
    ...
    void VulkanDriver::createRenderPass()
    {
        ...
        colorAttachment.finalLayout = VK_IMAGE_LAYOUT_PRESENT_SRC_KHR; //To be presented in swap chain

        VkAttachmentReference colorAttachmentRef{};
        colorAttachmentRef.attachment = 0; //Index in renderpas's array of attachments
        ...
        //To take care of the transition at the start of the render pass and at the end of the render pass
    ...
    void VulkanDriver::createVertexBuffer()
    {
        ...
        createBufferInternal(bufferSize, VK_BUFFER_USAGE_TRANSFER_SRC_BIT,
        ...
        createBufferInternal(bufferSize, VK_BUFFER_USAGE_TRANSFER_DST_BIT | VK_BUFFER_USAGE_VERTEX_BUFFER_BIT,
    ...
    void VulkanDriver::createIndexBuffer()
    {
        ...
        createBufferInternal(bufferSize, VK_BUFFER_USAGE_TRANSFER_DST_BIT | VK_BUFFER_USAGE_INDEX_BUFFER_BIT,
    ...
    void VulkanDriver::createBufferInternal(VkDeviceSize size, VkBufferUsageFlags usage, VkMemoryPropertyFlags properties, VkBuffer& buffer, VkDeviceMemory& bufferMemory) {
    ...
    void VulkanDriver::createSyncObjects()
    {
        ...
        fenceInfo.flags = VK_FENCE_CREATE_SIGNALED_BIT; //Ceate already signalled
    ...
    {
        //Wait until frame with this index is drawn
        vkWaitForFences(device, 1, &inFlightFences[currentFrame], VK_TRUE, 10000000000);
        ...
        vkResetFences(device, 1, &inFlightFences[currentFrame]);//Will be set again after vkQueueSubmit finifhes
        ...
        //Present image back to swap chain
        ...
        presentInfo.pResults = nullptr; // Optional for multi swap chains

        
        VkResult presentResult = vkQueuePresentKHR(presentQueue, &presentInfo);

        //Catch window resize here too
        if (presentResult == VK_ERROR_OUT_OF_DATE_KHR || presentResult == VK_SUBOPTIMAL_KHR || isFramebufferResized) {
            isFramebufferResized = false;
            recreateSwapChain();
    ...
    void VulkanDriver::updateUniformBuffer(uint32_t currentImageIndex)
    {
    ...
    void VulkanDriver::createDepthResources()
    {
        ...
        imageUtils.createImage(swapChainUtils.GetExtent().width, swapChainUtils.GetExtent().height,
         depthFormat,
        ...
        depthImageView = imageUtils.createImageView(depthImage, depthFormat, VK_IMAGE_ASPECT_DEPTH_BIT);
    }
